    ...
    def __init__(
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(model_dim, Attention(model_dim, num_heads=num_heads, head_dim=head_dim, dropout=attn_dropout)),
                PreNorm(model_dim, FeedForward(model_dim, dropout=ff_dropout)),
            ]))
        ...
        *,
        ...
        if self.num_continuous > 0:
            if exists(continuous_mean_std):
                assert continuous_mean_std.shape == (num_continuous, 2), f'continuous_mean_std must have a shape of ({num_continuous}, 2)'
                self.register_buffer('continuous_mean_std', continuous_mean_std)
        else:
            self.register_buffer('continuous_mean_std', torch.zeros(num_continuous, 2))
